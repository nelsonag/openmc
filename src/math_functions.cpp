...
void calc_rn_c(int n, double uvw[3], double rn[]){
  ...
  switch (n) {
    ...
    case 2:
      // l = 2, m = -2
      rn[0] = 0.288675134594813 * (-3. * w * w + 3.) * sin(2. * phi);
      // l = 2, m = -1
      rn[1] = -(1.73205080756888 * w*std::sqrt(w2m1) * sin(phi));
      // l = 2, m = 0
      rn[2] = 1.5 * w * w - 0.5;
      // l = 2, m = 1
    ...
    case 3:
      // l = 3, m = -3
      rn[0] = -(0.790569415042095 * std::pow(w2m1, 1.5) * sin(3. * phi));
      // l = 3, m = -2
      rn[1] = 1.93649167310371 * w*(w2m1) * sin(2.*phi);
      // l = 3, m = -1
    ...
    case 4:
      // l = 4, m = -4
      rn[0] = 0.739509972887452 * (w2m1 * w2m1) * sin(4.0*phi);
      // l = 4, m = -3
      rn[1] = -(2.09165006633519 * w * std::pow(w2m1, 1.5) * sin(3.* phi));
      // l = 4, m = -2
      rn[2] = 0.074535599249993 * (w2m1)*(52.5 * w * w - 7.5) * sin(2. *phi);
      // l = 4, m = -1
    ...
    case 5:
      // l = 5, m = -5
      rn[0] = -(0.701560760020114 * std::pow(w2m1, 2.5) * sin(5.0 * phi));
      // l = 5, m = -4
      rn[1] = 2.21852991866236 * w * w2m1 * w2m1 * sin(4.0 * phi);
      // l = 5, m = -3
      rn[2] = -(0.00996023841111995 * std::pow(w2m1, 1.5) *
           ((945.0 /2.)* w * w - 52.5) * sin(3.*phi));
      // l = 5, m = -2
      rn[3] = 0.0487950036474267 * (w2m1)
           * ((315.0/2.)* std::pow(w, 3) - 52.5 * w) * sin(2.*phi);
      // l = 5, m = -1
      rn[4] = -(0.258198889747161*std::sqrt(w2m1) *
           (39.375 * std::pow(w, 4) - 105.0/4.0 * w * w + 15.0/8.0) * sin(phi));
      // l = 5, m = 0
    ...
    case 6:
      // l = 6, m = -6
      rn[0] = 0.671693289381396 * std::pow(w2m1, 3) * sin(6.0*phi);
      // l = 6, m = -5
      rn[1] = -(2.32681380862329 * w*std::pow(w2m1, 2.5) * sin(5.0*phi));
      // l = 6, m = -4
      rn[2] = 0.00104990131391452 * w2m1 * w2m1 *
           ((10395.0/2.) * w * w - 945.0/2.) * sin(4.0 * phi);
      // l = 6, m = -3
      rn[3] = -(0.00575054632785295 * std::pow(w2m1, 1.5) *
           ((3465.0/2.) * std::pow(w, 3) - 945.0/2.*w) * sin(3.*phi));
      // l = 6, m = -2
    ...
    case 7:
      // l = 7, m = -7
      rn[0] = -(0.647259849287749 * std::pow(w2m1, 3.5) * sin(7.0*phi));
      // l = 7, m = -6
      rn[1] = 2.42182459624969 * w*std::pow(w2m1, 3) * sin(6.0*phi);
      // l = 7, m = -5
      rn[2] = -(9.13821798555235e-5*std::pow(w2m1, 2.5) *
           ((135135.0/2.)*w * w - 10395.0/2.) * sin(5.0*phi));
      // l = 7, m = -4
      rn[3] = 0.000548293079133141 * w2m1 * w2m1 *
           ((45045.0/2.)*std::pow(w, 3) - 10395.0/2.*w) * sin(4.0*phi);
      // l = 7, m = -3
      ...
      rn[6] = -(0.188982236504614*std::sqrt(w2m1) *
           ...
           (945.0/16.0)*w * w - 35.0/16.0) * sin(phi));
      // l = 7, m = 0
    ...
    case 8:
      // l = 8, m = -8
      rn[0] = 0.626706654240044 * std::pow(w2m1, 4) * sin(8.0*phi);
      // l = 8, m = -7
      rn[1] = -(2.50682661696018 * w*std::pow(w2m1, 3.5) * sin(7.0*phi));
      // l = 8, m = -6
      rn[2] = 6.77369783729086e-6*std::pow(w2m1, 3)*
           ((2027025.0/2.)*w * w - 135135.0/2.) * sin(6.0*phi);
      // l = 8, m = -5
      rn[3] = -(4.38985792528482e-5*std::pow(w2m1, 2.5) *
                ((675675.0/2.)*std::pow(w, 3) - 135135.0/2.*w) * sin(5.0*phi));
      // l = 8, m = -4
    ...
    case 10:
      // l = 10, m = -10
      rn[0] = 0.593627917136573 * std::pow(w2m1, 5) * sin(10.0 * phi);
      // l = 10, m = -9
      rn[1] = -(2.65478475211798 * w * std::pow(w2m1, 4.5) * sin(9.0 * phi));
      // l = 10, m = -8
      rn[2] = 2.49953651452314e-8 * std::pow(w2m1, 4) *
           ((654729075.0/2.) * w * w - 34459425.0/2.) * sin(8.0 * phi);
      // l = 10, m = -7
